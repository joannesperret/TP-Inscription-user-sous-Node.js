...
// Exposition des fichiers dans le dossier public
app.use('/register',express.static(__dirname + '/public'));

// Définition de la route d'enregistrement
app.get('/register', (req, res) =>{
  res.render('authentification');
 });

 // Ajout du nouvel utilisateur à la base de données, TP à faire:

 app.post('/user', (req, res) => {
  // Les données postées sont dans req.body
  const data = req.body;
  });
...
app.post('/register', async (req, res) => {
  try {
    ...
    const insert = await cn.query(
      'INSERT INTO users (login,pwd) VALUES (?, SHA1(?))',
    ...
    // Il faut créer un objet car JWT n'aime pas celui généré par la requête
      ...
      res.status(200).json({login: data.login, password: data.pwd, id: data.id, token: token});
...
app.get('/jwt', (req, res) => {
  ...
  const user = { name: 'Joannès', pass: '123', role: 'admin' };
  // Création d'un token
